...
module.exports.MoHinhNhua = function (req, res) {
    Product.find({ cateProduct: 'MoHinhNhua' }, function (err, data) {
        if (err) {
            res.json({ "kq": 0, "errMsg": err });
        } else {
            Session.findOne({ _id: req.session.views }, function (err, data1) {
                if (err) {
                    ...
                    res.render('./paperuser/MoHinhNhua', { listproduct: data, listdata: data1 })
                }
            });
        }
    })
}
module.exports.MoHinhGiay = function (req, res) {
    Product.find({ cateProduct: 'MoHinhGiay' }, function (err, data) {
        if (err) {
            res.json({ "kq": 0, "errMsg": err });
        } else {
            Session.findOne({ _id: req.session.views }, function (err, data1){
                if (err) {
                    ...
                    res.render('./paperuser/MoHinhGiay', { listproduct: data, listdata:data1 });
                }
            })
        }
    })
